...
DOM 은 HTML 문서의 계층적 구조와 정보를 표현하며 이를 제어할수있는 API 인 프로퍼티아 메서드를 제공하는 **트리 자료구조**이다.
HTML 을 DOM 과 연관지어 바라봐야한다.

## 중요한 노드 타입 12가지 중 4가지
...
getElementsByClassName 나 getElementsByTagName 을 반환한다.
...
# 39-21. querySelectorAll

querySelectorAll 메서드를 사용
...
스페이스 탭 줄바꿈개행의 공백 문자는 텍스트 노드를 생성한다.
...
Node.prototype.previouseSibling
...
Node.proptotype.nodeType
Node.proptotype.nodeName
...
setter 와 getter 모두 가능한 접근자 프로퍼티다.
...
Node.prototype.textContent 요소노드의 텍스트와 모든 자손 텍스트를 취득하거나 변경한다.
...
Element.prototype.innerHTML 은 setter getter 가능한 접근자 프로퍼티이며 HTML 마크업을 취득하거나 변경한다.

HTML 마크업이 포한된 문자열 그대로 반환한다.

## 단점

HTML 그대로 노출되기에, 크로스 사이트 스크립팅 공격에 취약하다.
이때 HTML sanitization 라이브러리를 사용하는것을 권장한다.
...
# 39-55. 텍스트 노드를 $li 요소 노드의 자식 노드로 추가
...
# 39-57. $li 요소 노드를 #fruits 요소 노드의 마지막 자식 노드로 추가
...
childe 매개변수에 인수로 전달한 노드를 DOM 에서 삭제한다.
